...
for i in sp_dict:
    s4[i]=(s3[i]/(s2[i]-s3[i]))   
li1=sorted(s4.items(), key=lambda item: item[1],reverse=True)[:3]
# print(li1)
li2=sorted(s4.items(), key=lambda item: item[1])[:3]
# print(li2)
for i in li1:
...
# print(snames)
# print(ratios)
tttwo = { 'State-codes': codes}
tttwo = pd.DataFrame(tttwo)
tttwo.to_csv('3-to-2-ratio.csv',index=False)
